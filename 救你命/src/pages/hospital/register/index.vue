  ...
  <div class="register">
    ...
    <div class="content">
      ...
      <div class="right">
        <div>挂号规则</div>
        <div class="time">
          预约周期: {{ hospitalStore.hospitalinfo.bookingRule?.cycle }}
          天 放号时间:
        ...
        <div class="releasetime">
          退号时间:就诊前一工作日
          {{ hospitalStore.hospitalinfo.hospital?.quitTime}}前取消
        </div>
    ...
    <div class="deparment">
      <div class="leftNav">
        <ul>
          <li
            v-for="(deparment,index) in hospitalStore.deparmentArr.data"
      ...
      <div class="deparmentInfo">
        ...
        <div
          class="showDeparment"
          v-for="deparment in hospitalStore.deparmentArr.data"
          :key="deparment.depcode"
        >
          <h1 class="cur">{{ deparment.depname }}</h1>
          <!-- 每一个大的科室下小科室 -->
          <ul>
            <li  v-for="item in deparment.children" :key="item.depcode" @click="login(item)" style="cursor: pointer;">
              {{ item.depname }}
            </li>
          </ul>
        </div>
...
let hospitalStore:any = useDetailStore();
import { ref } from "vue";
