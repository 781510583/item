  ...
  <div class="wrap">
    ...
    <div class="bottom">
      ...
      <div class="will" v-if="workTime.status == 1">
        <span class="time">2023年9月3日08:30</span>
...
import { guahaoAPI,doctorAPI } from "@/api/hospital";
//获取路由对象
let $route = useRoute();
//获取路由器对象
...
const guahaoData = async () => { 
  let res = await guahaoAPI(
    pageNo.value,
    limit.value,
...
const doctorData = async () => {
  ...
  //获取排班医生的数据
  let res = await doctorAPI(hoscode, depcode, workDate);
  // console.log(res);
  if (res.data.code == 200) {
    docArr.value = res.data.data;
  }
};
...
// //路由跳转进入到选择就诊人页面
const goStep2 = (doctor) => {
  //编程式导航进行路由跳转且携带医生的ID
  docinfo.value = doctor
