          ...
          <div class="login">
            <div v-show="scene == 0">
              <el-form :model="loginParam" ref="form" :rules="rules">
                ...
                <el-form-item>
                  <el-button
                    :disabled="!truephone || count ? true : false"
                    @click="getCode"
                  >
                    <countdown
                      v-if="count"
              ...
              <div class="bottom" @click="changeScene">
                <p style="cursor: pointer">微信扫码登录</p>
                <svg
                  t="1685263287521"
                  class="icon"
            ...
            <div class="webchat" v-show="scene == 1" style="cursor: pointer">
              <!-- 在这个容器当中显示微信扫码登录页面 -->
...
//@ts-ignore
import countdown from "@/components/count/index.vue";
import { wxAPI } from "@/api/hospital/index";
...
let loginParam: any = reactive({
  phone: "",
  code: "",
});
let count: any = ref<boolean>(false);
let userStore: any = useUserStore();
let scene = ref<number>(0);
async function changeScene() {
  scene.value = 1;
  let redirect_URL = encodeURIComponent(window.location.origin + "/wxlogin");
  const res = await wxAPI(redirect_URL);
  console.log(res);
//@ts-ignore
  new WxLogin({
...
const getCode = async () => {
  count.value = true;
  userStore.getphonecode();
  loginParam.code = userStore.code;
};
function getcount(val: boolean) {
  count.value = val;
}
...
function close() {
  (loginParam.phone = ""), (loginParam.code = ""), form.value.resetFields();
}
